...
export default {
  name: 'studentFillProjectTrainningReportFinel',
  data() {
    return {
      list: [],
      showTable: false,
  ...
  methods: {
    fetchData() {
      fillProjectTrainningReportInit({ 'reportType': '2' }).then(response => {
        if (response.re == 1) {
          this.showTable = true
    ...
    projectSubmit() {
      fillProjectTrainningReport({ 'reportType': '2', 'reportId': this.reportId, 'reportContent': this.list.reportContent }).then(response => {
